<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	System.out.println(String.format(user_list_query, userid));
	try{
		Class.forName("oracle.jdbc.driver.OracleDriver");
		Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "min", "1234");
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt.executeQuery(String.format(user_list_query, userid));
		while (rs.next()) {
	%>
<div class="input-container">
	<form action="../action/user-edit.jsp">
		<div class="form-floating mb-3">
		  <input type="hidden" class="form-control" id="floatingInput" placeholder="name@example.com" name="userid" value="<%=userid %>">
		  <label for="floatingInput">책 번호</label>
		</div>
		<fieldset disabled>
		<div class="form-floating mb-3">
		  <input type="text" class="form-control" id="disabledTextInput" placeholder="name@example.com" value="<%=userid %>">
		  <label for="floatingInput">유저 아이디</label>
		</div>
		</fieldset>
	  <div class="form-floating mb-3">
		  <input type="text" class="form-control" id="floatingInput" placeholder="name@example.com" name="userpwd" value="<%=rs.getString("userpwd") %>">
		...
		<div class="form-floating mb-3">
		  <input type="text" class="form-control" id="floatingPassword" placeholder="admin_ck" name="admin_ck" value="<%=rs.getString("admin_ck") %>">
